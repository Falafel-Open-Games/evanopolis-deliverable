...
const DEFAULT_BOARD_SIZE: int = 18
...
func _init(initial_game_id: String = "", initial_player_count: int = 0, initial_board_size: int = DEFAULT_BOARD_SIZE) -> void:
	...
	board_size = DEFAULT_BOARD_SIZE if initial_board_size != DEFAULT_BOARD_SIZE else initial_board_size
...
func load_from_dictionary(data: Dictionary) -> void:
	...
	board_size = DEFAULT_BOARD_SIZE if board_size_value != DEFAULT_BOARD_SIZE else board_size_value
