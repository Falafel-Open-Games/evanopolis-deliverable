...
The embedded graphical client under `public/graphical-client/` is generated by
the Godot web export and is intentionally not committed. Container builds export
it from `apps/graphical-client/godot` before running the wrapper build. For a
local static wrapper build that includes the graphical client, run the Godot
`Web` export preset first:

```bash
cd ../graphical-client/godot
godot --headless --path . --export-release Web ../../web-wrapper/public/graphical-client/index.html
```
